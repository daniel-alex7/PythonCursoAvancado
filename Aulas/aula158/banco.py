...
class Banco:
    ...
    def _checa_agencia(self, conta):
        if conta.agencia in self.agencias:
            return True
        print('_checa _agencia', True)
        return False
        
    def _checa_cliente(self, cliente):
        if cliente in self.clientes:
            return True
        print('_checa _cliente', True)
        return False
        
    def _checa_conta(self, conta):
        if conta in self.contas:
            return True
        print('_checa _conta', True)
        return False
...
if __name__ == '__main__':
    c1 = pessoas.Cliente('Daniel', 20)
    cc1 = contas.ContaCorrente(111, 222, 0, 0)
    c1.conta = cc1
    c2 = pessoas.Cliente('Juliana', 20)
    cp1 = contas.ContaPoupanca(777, 444, 1000)
    c2.conta = cp1
    banco = Banco()
